...
static void parse(T& nums, const char** argv) {
...
template <template <typename> class Container, typename T>
static void createPairs(Container<T>& nums, Container<std::pair<T, T> > pairs) {
...
template <template <typename> class Container, typename T>
static void binarySearch(Container<T>& S, const int number) {
...
static void sortLosers(Container<T> S, const size_t& n,
  ...
  if (pairs.size() > 0 && pairs[0].first != UNUSED)
    binarySearch(S, pairs[0].first);

  for (size_t i = 1; i < jacobSequence.size(); i++) {
    for (size_t k = jacobSequence[i]; k > jacobSequence[i - 1]; k--) {
      if (k >= pairs.size() || pairs[k].first == UNUSED) continue;
      binarySearch(S, pairs[k].first);
      if (S.size() == n) return;
    }
  }
}
